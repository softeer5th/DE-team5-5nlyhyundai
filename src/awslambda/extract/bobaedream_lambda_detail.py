...
def lambda_handler(event, context):
    ...
    if details_data == 500:
        raise "bobae detail: 500 - [ERROR] DETAIL / DB 연결 실패"
        return {
            "status_code": 500,
    ...
    try:
        ...
        raise "bobae detail: 500 - [ERROR] S3 저장 실패"
        return {
            "status_code": 500,
            "body": "[ERROR] S3 저장 실패"
        }
        ...
        if status_code == 403:
            raise f"bobae detail: 403 - [WARNING] DETAIL / IP 차단됨 / 크롤링 데이터: {len(details_data)} 건"
            return {
                "status_code": 403,
                "body": f"[WARNING] DETAIL / IP 차단됨 / 크롤링 데이터: {len(details_data)} 건"
            }
        elif status_code == 408:
            raise f"bobae detail: 403 - [WARNING] DETAIL / 람다 함수 시간 초과 / 크롤링 데이터: {len(details_data)} 건"
            return {
                "status_code": 408,
