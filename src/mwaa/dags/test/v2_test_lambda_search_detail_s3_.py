...
def generate_lambda_detail_configs(**context) -> List[dict]:
    ...
    function_names_str = Variable.get('TEST_LAMBDA_DETAIL_FUNC')
    ...
    detail_batch_size = int(Variable.get('DETAIL_BATCH_SIZE')) # 현재 40개로 설정. 배치 더 작게 해도 괜찮음. 어차피 라운드 로빈.
...
with DAG(
    'v2_test_lambda_to_s3_workflow',
    default_args=default_args,
    description='[TEST] (V2) Trigger Lambda functions and Load data to S3',
    ...
    # set_s3_sensor = S3KeySensor(
    #     task_id='set_s3_sensor',
    #     bucket_name='extract-web',
    #     bucket_key="{{ datetime.strftime(datetime.strptime(task_instance.xcom_pull(task_ids='set_environment', key='checked_at'), '%Y-%m-%dT%H:%M:%S'), '%Y-%m-%d/%H/%M') }}",
    #     timeout=1*15*60,
    #     poke_interval=20,
    #     mode='poke',
    #     dag=dag
    # )
    # 비동기, s3에 파일이 올라간 이후 시간 제한 두기 (batch에 따라 비동기일 경우 시간이 달라짐!)

    trigger_second_dag = TriggerDagRunOperator(
        task_id='trigger_test_emr_transform_dag',
        trigger_dag_id='test_emr_transform_dag',
        conf={
            'from_dag': 'v2_test_lambda_to_s3_workflow',
