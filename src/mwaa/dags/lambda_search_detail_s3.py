...
def generate_lambda_detail_configs(**context) -> List[dict]:
    ...
    with pg_hook.get_conn() as conn:
        with conn.cursor() as cursor:
            cursor.execute('''
                        SELECT 
                            (SELECT COUNT(*) FROM probe_bobae WHERE status not in ('UNCHANGED', 'BANNED')) as bobae_count,
                            (SELECT COUNT(*) FROM probe_clien WHERE status not in ('UNCHANGED', 'BANNED')) as clien_count,
                            (SELECT COUNT(*) FROM probe_dcmotors WHERE status not in ('UNCHANGED', 'BANNED')) as dc_count
                           ;
                        ''')
    ...
    # 20개 당 하나씩 처리
    detail_batch_size = int(Variable.get('DETAIL_BATCH_SIZE')) # 현재 40개로 설정. (어차피 그 이하는 키는 비용이 더욱 나감)
